...
"""path = "ToMinistery/drive/*.CR2" #rawfile #o seu arquivo direto da camera.
fotos = glob.glob(path)

for file in fotos:
    raw = rawpy.imread(file)
    rgb = raw.postprocess(use_auto_wb=True)
    imageio.imsave(file+'.jpg', rgb)
...
for filename in filenames:
        ...
        cv2.imwrite("ToMinistery/drive/gray/ministery_gray_" +str(count) +".jpg", gray_img)
        cv2.imwrite("ToMinistery/drive/color/ministery_color_" +str(count) +".jpg", image)
